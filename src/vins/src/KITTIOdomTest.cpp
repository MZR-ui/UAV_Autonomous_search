...
// Convert a mono8 cv::Mat to a sensor_msgs::ImagePtr without cv_bridge,
// to avoid OpenCV version coupling (cv_bridge is prebuilt against OpenCV 4.2).
static sensor_msgs::ImagePtr matToMono8(const cv::Mat &img, const std_msgs::Header &header)
{
...
int main(int argc, char** argv)
{
	...
	{	
		if(ros::ok())
		{
			...
			sensor_msgs::ImagePtr imLeftMsg = matToMono8(imLeft, std_msgs::Header());
			imLeftMsg->header.stamp = ros::Time(imageTimeList[i]);
			pubLeftImage.publish(imLeftMsg);

			imRight = cv::imread(rightImagePath, CV_LOAD_IMAGE_GRAYSCALE );
			sensor_msgs::ImagePtr imRightMsg = matToMono8(imRight, std_msgs::Header());
