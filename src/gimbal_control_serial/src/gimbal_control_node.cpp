...
#include "gimbal_control_serial/GimbalStatus.h"
#include <vector>
#include <mutex>
...
public:
        ...
        : nh_(nh)
    {
        ...
        if (!serial_->open()) {
            ROS_ERROR_THROTTLE(1.0, "Failed to open serial port: %s", port_name_.c_str());
            // ros::shutdown(); // Removed to allow auto-reconnect
        } else {
        ...
        status_pub_ = nh_.advertise<gimbal_control_serial::GimbalStatus>("/gimbal/status", 10);

        serial_->setReceiveCallback([this](const Gbc2GcuPkt_t& pkt) {
            gimbal_control_serial::GimbalStatus msg;
            msg.header.stamp = ros::Time::now();
            msg.fw_ver = pkt.fw_ver;
            msg.hw_err = pkt.hw_err;
            msg.inv_flag = pkt.inv_flag;
            msg.gbc_stat = pkt.gbc_stat;
            msg.tca_flag = pkt.tca_flag;
            for(int i=0; i<3; i++) {
                msg.cam_rate[i] = pkt.cam_rate[i];
                msg.cam_angle[i] = pkt.cam_angle[i];
                msg.mtr_angle[i] = pkt.mtr_angle[i];
            }
...
private:
    ...
    ros::Timer timer_;
    std::unique_ptr<GimbalSerial> serial_;   // 串口句柄
    //std::shared_ptr<GimbalSerial> serial_;   // ? 锟侥筹拷指串口句柄
    ...
    std::mutex mutex_;
    gimbal_control_serial::GimbalCmd last_cmd_; // 存储最新接收到的控制指令
    ...
    void timerCallback(const ros::TimerEvent&)
    {
        if (!serial_->isOpen()) {
            ...
            ROS_INFO("Successfully reconnected to serial port %s", port_name_.c_str());
        }

        Gcu2GbcPkt_t pkt = {0};
        memset(&pkt, 0, sizeof(pkt)); // 清空数据
        ...
        {
            std::lock_guard<std::mutex> lock(mutex_);
            // 将命令映射至 roll/pitch/yaw  uav.angle
            pkt.uav.valid = 0;
        ...
        if (!serial_->sendPacket(pkt)) {
            // ROS_WARN_THROTTLE(1.0, "Failed to send packet to gimbal");
        }
        ...
        //if (serial_.readPacket(recv_pkt))  // 阻塞读取，需要在 gimbal_serial.cpp 中实现
        //{
            // 打印接收到的数据
